...
dentrite_edges = [[(c.start.y,c.start.x),(c.end.y,c.end.x)] for c in mcn.compartments if not c.axon_comp]
soma_edges = [[(c.start.y,c.start.x),(c.end.y,c.end.x)] for c in mcn.compartments if c.axon_comp]

print dentrite_edges
print soma_edges

lc_dentrite = mc.LineCollection(dentrite_edges, colors='k', linewidths=1)
lc_soma = mc.LineCollection(soma_edges, colors='r', linewidths=1)

ax = plt.axes()
ax.add_collection(lc_dentrite)
ax.add_collection(lc_soma)
plt.xlim([-500,500])
plt.ylim([-2000,20])
